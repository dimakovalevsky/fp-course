...
tok ::
  ...
  -> Parser a
tok p = p <* spaces
-- tok p = P (\x -> case parse p x of
--                    Result li a -> onResult (parse spaces li) (\y _ -> Result y a)
--                    a -> a)
...
-- oneof Nil = P (const UnexpectedEof)
-- oneof (h :. t) = is h ||| oneof t
...
hex ::
  Parser Char
hex = (\x -> let hx = readHex x in
               case hx of
               Full a -> pure . chr $ a
               Empty  -> P UnexpectedString) =<< thisMany 4 (satisfy isHexDigit)
...
eof ::
  Parser ()
eof = P (\x -> if isEmpty x then Result Nil () else ExpectedEof x)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll ps = satisfy (\c -> and (ps <*> pure c))
-- satisfyAll ps = (\c -> (\a -> and ((\p -> p c) <$> a)) <$> sequence ps) =<< character
-- satisfyAll ps = satisfy (\c -> and ((\p -> p c) <$> ps))
-- satisfyAll ps = (\c -> if and ((\p -> p c) <$> ps) then pure c else unexpectedCharParser c) =<< character
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny ps = satisfy (\c -> or (ps <*> pure c))
-- satisfyAny =
