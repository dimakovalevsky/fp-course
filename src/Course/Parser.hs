...
(|||) ::
  ...
  -> Parser a
  -> Parser a
p1 ||| p2 = P (\x -> let p = if isErrorResult (parse p1 x) then p2 else p1 in parse p x)
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  f =<< (P g) = P (\x -> onResult (g x) (\y z -> parse (f z) y))
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  -- (P f) <*> p = P (\x -> onResult (f x) (\y z -> parse (z <$> p) y))
  f <*> p = (<$> p) =<< f
  -- f <*> (P g) = (\x -> P (\y -> onResult (g y) (\a b -> Result a (x b)))) =<< f
...
-- P (\x -> let val = parse p x in if isErrorResult val then onResult val (\a b -> Result a (b :. Nil)) else Result x Nil)

  -- P (\x -> let res = f x
  --                           isError = isErrorResult(res)
  --                           in if onResult res (\a b ->
  --                             Result (if isError then x else a) (if isError then iterate )))
...
smokerParser = (\case
                'y' -> valueParser True
                'n' -> valueParser False
                x   -> unexpectedCharParser x) =<< (is 'y' ||| is 'n')
...
phoneParser ::
  Parser Chars
phoneParser = digit .:. lift2 (++) phoneBodyParser (const (valueParser Nil) =<< is '#')
